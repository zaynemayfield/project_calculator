...
<!-- Project Details -->
        <div class="row mb-3" id="editMaterial">
            <div class="col">
                <h3><strong>{{ project.name }}</strong> - {{ project.summary }} <i>({{ project.type }})</i></h3>
            </div>
        </div>
<!-- END Project Details -->
 ...
 <div class="card shadow" v-if="showEditMaterialForm">
    <div class="card-body">
      ...
      <div class="row align-items-center">
          <div class="col">
            <!-- NAME FIELD -->
          <input type="text" class="form-control" id="name" name="name" placeholder="Name" :value="updatedMaterial.material.name" required />
        </div>
        <div class="col">
            <!-- URL FIELD -->
          <input type="url" class="form-control" id="url" name="url" placeholder="URL"  :value="updatedMaterial.material.url" />
        </div>
        <div class="col">
            <!-- DESCRIPTION FIELD -->
          <input type="text" class="form-control" id="description" name="description" placeholder="Description"  :value="updatedMaterial.material.description" />
        </div>
        <div class="col">
            <!-- PRICE FIELD -->
          <input type="number" class="form-control" min="0.00" step="0.01" id="price" name="price" placeholder="0.00" :value="updatedMaterial.material.price" />
        </div>
        <div class="col">
            <input type="hidden" name="id" :value="updatedMaterial.material_id">
            <!-- SUBMIT -->
...
<div class="border rounded p-2" id="checkDelete">
    <div class="row">
        ...
        <div class="col">
            Number of Items: {{ materialLines.length }}
        </div>
    </div>
</div>
...
<div class="alert alert-danger" role="alert" v-if="showCheckDeleteForm">
  Are you sure you want to delete material? Any items associated with this material will be deleted in this project. <br>
  {{ checkDeleteMaterial.material.name}} - {{ checkDeleteMaterial.material.description}} <br>
  <div class="row mt-2">
    <div class="col-auto">
        <button type="submit" class="btn btn-success" @click="showCheckDeleteForm = false">No, Cancel Delete</button> 
    </div>
    <div class="col-auto">
        <button type="submit" class="btn btn-danger" @click="deleteMaterial(checkDeleteMaterial.material_id)">Yes, Delete Material</button>
    </div>      
  </div>
...
<!-- If no Materials -->
<div class="border rounded p-3" v-if="!materialLines">
...
<div class="border rounded p-3 mb-2" :key="index" v-for="(material, index) in materialLines">
    <form enctype="application/x-www-form-urlencoded" @submit.prevent="updateLineItem">
    <!-- TOP ROW -->
        <div class="row  align-items-center mb-2">

            <!-- NAME/URL/DESCRIPTION for Material -->
            <div class="col">
                <a :href="material.material.url" target="_blank"> {{  material.material.name }} </a> - {{  material.material.description }}
            </div>

            <!-- PRICE -->
            <div class="col">
                {{  financialFormat(material.material.price) }}ea x {{ material.quantity }} = {{financialFormat(material.quantity * material.material.price) }}
            </div>

            <!-- EDIT MATERIAL -->
            <div class="col">
                <div class="btn-group" role="group" aria-label="Basic example">
                <button @click="beginMaterialEdit(material, 'editMaterial')" class="btn btn-primary pr-3">Edit Material</button>
                <button class="btn btn-danger" @click="showDeleteMaterial(material, 'checkDelete')">Delete Material</button>
                </div>
            </div>
        ...
        <div class="w-100"></div>
            ...
            <div class="col">
                ...
                <input type="text" class="form-control-inline" id="notes" name="notes" placeholder="Notes" v-model="material.notes" />
                </div>
            </div>
            ...
            <div class="col">
                ...
                <input type="number" class="form-control-inline" min="0" step="1" id="quantity" name="quantity" placeholder="Quantity" v-model="material.quantity" />
            ...
            <div class="col">
                ...
                <button @click="duplicateLineItem(material.id, 'checkDelete')" class="btn btn-primary" type="button"> Duplicate Item&nbsp;&nbsp;  </button>
                <button class="btn btn-danger" type="button" @click="deleteLineItem(material.id)">Delete Item</button>
                </div>
            </div>
        </div>
        </form>
...
// get all materials and line items associated with project ID
        const getMaterialLines = async () => {
            materialLines.value = await apiClient.getMaterialLines(projectId)
        }
//Create new material along with blank line item and then run getMaterialLines again
...
//Calculate the total cost of all line items in project
        const totalProjectCost = () => {
            let totalCost = 0
            for (const lineItem of materialLines.value) {
