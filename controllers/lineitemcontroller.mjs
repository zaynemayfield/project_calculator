...
async both (req, res) {
  ...
  const materialLine = await prisma.line_item.findMany({ where: { project_id: project_id}, include: { material: { select: { name: true, url: true, price: true, description: true }}}, orderBy: { updatedAt: 'desc' }})
  if (!materialLine) {
...
async delete (req, res) {
  const id = parseInt(req.params.id)
  try {
  const lineItem = await prisma.line_item.delete({ where: {id: id}})
  const deleteLineItem = id
  return res.send({deleteLineItem: deleteLineItem})
  } catch (error) {
    console.log(error)
  }
