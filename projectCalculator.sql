...
CREATE DATABASE IF NOT EXISTS `project_calculator` /*!40100 DEFAULT CHARACTER SET utf8mb3 */;
...
CREATE TABLE IF NOT EXISTS `line_item` (
  ...
  KEY `user_id on line_item` (`user_id`),
  ...
  CONSTRAINT `user_id on line_item` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE IF NOT EXISTS `material` (
  ...
  KEY `project_id on material` (`project_id`),
  CONSTRAINT `project_id on material` FOREIGN KEY (`project_id`) REFERENCES `project` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS `project` (
  ...
  `updatedAt` timestamp NULL DEFAULT current_timestamp(),
  `name` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL,
  `type` enum('Public','Private','Friends') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'Public',
  `summary` varchar(2000) COLLATE utf8mb4_unicode_ci NOT NULL,
  `user_id` int(10) unsigned NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `no duplicate projects` (`name`,`user_id`),
...
CREATE TABLE IF NOT EXISTS `user` (
  ...
  `type` enum('user','admin','mod') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'user',
  PRIMARY KEY (`id`),
...
-- Dumping structure for table project_calculator._prisma_migrations
